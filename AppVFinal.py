...
class MainScreen(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.model_images = {
            "MODEL 1": tk.PhotoImage(file="model1OK.png").subsample(5),
            "MODEL 2": tk.PhotoImage(file="model2OK.png").subsample(5),
            "MODEL 3": tk.PhotoImage(file="model3OK.png").subsample(5)
        }
    ...
    def confirm(self):
        if self.cropped_image:
            ...
            self.cropped_image.save(os.path.join(dir_base,image_path))  # Guardar la imagen recortada en un archivo temporal
        else:
            image_path = "uploaded_image.png"  # Ruta de archivo temporal para la imagen cargada
            self.image.save(os.path.join(dir_base,image_path))  # Guardar la imagen cargada en un archivo temporal 


            self.loading_screen = LoadingScreen(self)        
            self.execute_style(image_path)
    ...
    def execute_style(self, image_ruta):
        image_path = image_ruta        
        if self.model_images[self.model_var.get()]=="MODEL 1":
            style_image_path = "model1.png"
        elif  self.model_images[self.model_var.get()]=="MODEL 2":
            style_image_path = "model2.png"
        else:
            style_image_path = "model3.png"
        niter = self.quality_scale.get()
        style_transfer = StyleTransfer()
        output_image = style_transfer.aplica_estilo2(image_path, style_image_path, niter)
        keras.preprocessing.image.save_img(os.path.join(dir_base,"result.png"), output_image)
...
class LoadingScreen(tk.Frame):
    ...
    def start_process(self):
        ...
        output_image = style_transfer.aplica_estilo(image_path, image_path, style_image_path, 100)
...
class StyleTransfer:
    ...
    def aplica_estilo2(self, base_image_path, model_path,iterations):       
        # Build a VGG19 model loaded with pre-trained ImageNet weights
        model = vgg19.VGG19(weights="imagenet", include_top=False)
        ...
        #display(Image("_at_iteration_%d.png" % i))
...
class ResultScreen(tk.Frame):
    ...
    def download(self):
        # Obtener la ruta de la imagen de salida
        output_image_path = os.path.join(dir_base, "result.png")
    
        # Obtener la ruta de descarga especificada por el usuario
        download_path = filedialog.asksaveasfilename(defaultextension=".png")
    
        # Verificar si se seleccionó una ruta de descarga válida
        if download_path:
            # Copiar la imagen a la ubicación de descarga
            shutil.copyfile(output_image_path, download_path)
